...
if command -v mise /dev/null 2>&1; then
  ...
  echo "deb [signed-by=/etc/apt/keyrings/mise-archive-keyring.gpg arch=amd64,arm64,armhf] https://mise.jdx.dev/deb stable main" | sudo tee /etc/apt/sources.list.d/mise.list  
  sudo apt update
